...
func HandleProfile(w http.ResponseWriter, r *http.Request) {
	...
	err = db.QueryRow("SELECT id, username, email, is_admin, is_moderator FROM users WHERE id = ?", userID).Scan(&user.ID, &user.Username, &user.Email, &user.IsAdmin, &user.IsModerator)
	if err != nil {
